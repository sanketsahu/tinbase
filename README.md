...
| Install size | 92 MB (58 MB binary + PG) | 36 MB³ | 6.7 MB³ | 27 MB³ | 30 MB | 2,291 MB⁴ |
...
³ Full production `node_modules` + `dist`, excluding the Node runtime you already have. Native: `dist` + node-pg (Postgres binaries download to a shared cache on first run). pg-mem: `dist` + `@tinbase/pg-mem` and its deps — ~6.7 MB (the fork uses a zero-dependency date layer; no `moment`). Wasm: `dist` + `@electric-sql/pglite`.
